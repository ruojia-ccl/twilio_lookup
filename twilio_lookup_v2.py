...
class SnowflakeCache:
    ...
    def _load_from_snowflake(self) -> dict:
        ...
        try:
            ...
            numbers = df.set_index("NUMBER_FORMATTED")[["PHONE_CARRIER", "REACHABLE_STATUS"]].apply(tuple, axis=1).to_dict()
            return numbers
    ...
    def _load_from_parquet(self) -> dict:
        df = pd.read_parquet(self.PARQUET_PATH, columns=["NUMBER_FORMATTED", "PHONE_CARRIER", "REACHABLE_STATUS"])
        numbers = dict(zip(df["NUMBER_FORMATTED"], zip(df["PHONE_CARRIER"], df["REACHABLE_STATUS"])))
    ...
    def return_carrier(self, phone):
        ...
        self._ensure_loaded()
        val = self._numbers.get(self._to_digits(phone), False)
        if val is False:
            return False
        return val[0]

    def return_cached(self, phone):
        """Return (carrier, reachable_status) from cache, or (False, None) if not found."""
        self._ensure_loaded()
        val = self._numbers.get(self._to_digits(phone), False)
        if val is False:
            return False, None
    ...
    def upload(self, df, conn=None, sid=None):
        """Upsert a DataFrame into Snowflake — updates existing rows by NUMBER_FORMATTED, inserts new ones."""
        required = {"NUMBER_FORMATTED", "PHONE_CARRIER", "PHONE_TYPE", "REACHABLE_STATUS"}
        ...
        close_conn = conn is not self._conn
        staging = "TWILIO_VALIDATED_NUMBERS_STAGING"

        try:
            cur = conn.cursor()

            # Create a session-scoped temp table — auto-dropped when connection closes
            cur.execute(f"""
                CREATE OR REPLACE TEMPORARY TABLE TWILIO_LOOKUPS.PUBLIC.{staging} (
                    NUMBER_FORMATTED  NUMBER,
                    PHONE_CARRIER     VARCHAR,
                    PHONE_TYPE        VARCHAR,
                    REACHABLE_STATUS  VARCHAR,
                    SID               VARCHAR
                )
            """)

            # Load batch into the temp table
            ...
            if not success:
                print("Failed to write to staging table — aborting upsert.")
                return

            # MERGE staging → main: update if exists, insert if new
            cur.execute(f"""
                MERGE INTO TWILIO_LOOKUPS.PUBLIC.TWILIO_VALIDATED_NUMBERS AS target
                USING TWILIO_LOOKUPS.PUBLIC.{staging} AS source
                ON target.NUMBER_FORMATTED = source.NUMBER_FORMATTED
                WHEN MATCHED THEN UPDATE SET
                    target.PHONE_CARRIER     = source.PHONE_CARRIER,
                    target.PHONE_TYPE        = source.PHONE_TYPE,
                    target.REACHABLE_STATUS  = source.REACHABLE_STATUS,
                    target.SID               = source.SID,
                    target.UPDATED_AT        = CURRENT_TIMESTAMP()
                WHEN NOT MATCHED THEN INSERT
                    (NUMBER_FORMATTED, PHONE_CARRIER, PHONE_TYPE, REACHABLE_STATUS, SID, UPDATED_AT)
                VALUES
                    (source.NUMBER_FORMATTED, source.PHONE_CARRIER, source.PHONE_TYPE,
                     source.REACHABLE_STATUS, source.SID, CURRENT_TIMESTAMP())
            """)

            result = cur.fetchone()
            rows_inserted = result[0] if result else 0
            rows_updated  = result[1] if result else 0
            print(f"Upsert complete: {rows_inserted} inserted, {rows_updated} updated.")
            cur.close()

        except Exception as e:
            print(f"Error upserting to Snowflake: {e}")
            raise
        finally:
            if close_conn:
                conn.close()
...
class TwilioLookup:
    ...
    def _upload_staging_file(self, conn=None):
        ...
        df["NUMBER_FORMATTED"] = df["NUMBER_FORMATTED"].astype(int)
        if "REACHABLE_STATUS" not in df.columns:
            df["REACHABLE_STATUS"] = None
        self.cache.upload(df, conn, sid=self.twilio.sid)
        os.remove(self.STAGING_PATH)
    ...
    async def _check_phone(self, session, phone, semaphore):
        ...
        url = f"{TWILIO_LOOKUP_BASE}/{clean_number}?Fields=line_status,line_type_intelligence"
            ...
            for attempt in range(self.twilio.max_retries):
                try:
                    async with session.get(url) as resp:
                        ...
                        line_status = data.get("line_status") or {}

                        return {
                            "phone": phone,
                            "clean_phone": clean_number,
                            "phone_type": line_type.get("type"),
                            "carrier": line_type.get("carrier_name"),
                            "reachable_status": line_status.get("status"),
                            "status": resp.status,
    ...
    async def validate(self, phones: List[str], conn=None, force: bool = False, stale_months: int | None = 12) -> Dict[str, pd.DataFrame]:
        ...
        force=True skips the cache entirely and calls Twilio for every number.
        stale_months re-validates cached numbers whose UPDATED_AT is older than
        that many months (default 12). Pass None to disable staleness checks.
        Results are uploaded to Snowflake, but since the upload is append-only,
        numbers already in the table will get duplicate rows. Run a dedup/MERGE
        in Snowflake afterward if you need clean data.
        """
        ...
        # --- Identify stale cached numbers that need re-validation ---
        stale_digits: set = set()
        if stale_months is not None and not force:
            try:
                stale_list = self.cache.stale_numbers(months=stale_months, conn=conn)
                stale_digits = {int(n) for n in stale_list if str(n).isdigit()}
                if stale_digits:
                    print(f"Will re-validate {len(stale_digits):,} cached numbers older than {stale_months} months.")
            except Exception as e:
                print(f"Warning: stale check failed ({e}), skipping staleness re-validation.")
        ...
        format_invalid_df = pd.DataFrame({
            ...
            "reachable_status": [None] * len(format_invalid),
        ...
        for p in unique_phones:
            carrier, reachable_status = (False, None) if force else self.cache.return_cached(p)
            if carrier is not False:
                digits = SnowflakeCache._extract_digits(p)
                if stale_digits and int(digits) in stale_digits:
        ...
        if uncached_phones:
            ...
            for r in twilio_results:
                ...
                if r["status"] == 200 and r["phone_type"] and r["carrier"]:
                    staging_rows.append({
                        ...
                        "REACHABLE_STATUS": r["reachable_status"],
                    })
            ...
            for r in twilio_results:
                ...
                if r["status"] == 200 and r["carrier"]:
                    self.cache._numbers[digits] = (r["carrier"], r["reachable_status"])
                elif 400 <= r["status"] <= 499:
                    self.cache._numbers[digits] = ("Not Valid", None)
        ...
        if not all_results and not cached_invalid:
            empty = pd.DataFrame(columns=["phone", "clean_phone", "phone_type", "carrier", "reachable_status", "status"])
            return {"valid": empty, "invalid": format_invalid_df, "failed": empty}

        df = pd.DataFrame(all_results) if all_results else pd.DataFrame()

        excluded_statuses = {"unreachable", "inactive"}

        valid_df = df[
            (df["status"] == 200) &
            (~df["reachable_status"].isin(excluded_statuses))
